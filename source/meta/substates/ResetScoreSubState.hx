...
class ResetScoreSubState extends MusicBeatSubstate
{
	...
	public function new(song:String, difficulty:Int, character:String, week:Int = -1)
	{
		...
		if(week > -1) {
			...
			if (name.toLowerCase() == 'inv?') name = decoyName + '?';
		} else
		name += ' (' + CoolUtil.difficultyStuff[difficulty][0] + ')?';
		...
		var text:Alphabet = new Alphabet(0, 180, (week != -1 ? "Reset the progress of" : "Reset the score of"), true);
		text.screenCenter(X);
	...
	override function update(elapsed:Float)
	{
		...
		if(controls.UI_LEFT_P || controls.UI_RIGHT_P) {
			FlxG.sound.play(Paths.sound('menus/base/scrollMenu'), 1);
			onYes = !onYes;
			updateOptions();
		}
		if(controls.BACK) {
			FreeplayState.disableControls = false;
			FlxG.sound.play(Paths.sound('menus/base/cancelMenu'), 1);
			close();
