...
class OptionsState extends MusicBeatState
{
	...
	function exitOptions(daSound:Bool = true) {
		if (daSound) FlxG.sound.play(Paths.sound('menus/base/cancelMenu'));
		if (onPlayState) {
		onPlayState = false;
...
class NotesSubstate extends MusicBeatSubstate
{
	...
	public function new() {
		...
		for (i in 0...ClientPrefs.arrowHSV.length) {
			...
			note.frames = Paths.getSparrowAtlas('notes/base/NOTE_assets');
			switch(i) {
				case 0:
...
class PreferencesSubstate extends MusicBeatSubstate
{
	...
	static var languageList:Array<String> = ['english', 'spanish', 'portuguese', 'french'];
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Anti-Mash':
						ClientPrefs.antiMash = !ClientPrefs.antiMash;
					...
					case 'Lunatic Mode':
						ClientPrefs.lunaticMode = !ClientPrefs.lunaticMode;
				}
				FlxG.sound.play(Paths.sound('menus/base/confirmMenu'));
				reloadValues();
			}
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				if (options[curSelected] == 'Language' && holdTime == 0) {
					...
					switch (languageNum) {
						...
						case 3:
							ClientPrefs.language = 'french';
					}
				}
				reloadValues();

				if(holdTime <= 0) FlxG.sound.play(Paths.sound('menus/base/scrollMenu'));
				holdTime += elapsed;
			} else {
				holdTime = 0;
			}
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Lunatic Mode':
				daText = 'Increases the mechanics difficulty and adds a Food System.\nRequiring you to maintain a high accuracy.';
			default:
				daText = "";
		}
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Lunatic Mode':
						daValue = ClientPrefs.lunaticMode;
				}
