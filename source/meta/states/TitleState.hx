...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(FlxG.save.data.flashing == null && !FlashingState.leftState) {
			...
			{
				var chance:Int = FlxG.random.int(0, 40);
				if (!seenIntro && !ClientPrefs.skipSplash) startVideo((chance == 0 ? 'secretIntro' : 'splash'));
				else startIntro();
	...
	function startIntro()
	{
		if (!initialized)
		{
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('rain'), 0);
	...
	//	OptionsState.PreferencesSubstate.updateFilter();
		...
		else //Psyka easter egg
		{
			...
			gfDance.frames = Paths.getSparrowAtlas('menus/title/psykaDanceTitle');
		...
		var rainLolllll = new FlxSprite();
		rainLolllll.frames = Paths.getSparrowAtlas('menus/title/rain');
		rainLolllll.animation.addByPrefix('idle', 'idle', 24, true);
		rainLolllll.screenCenter();
		add(rainLolllll);
		rainLolllll.animation.play('idle',true);
		...
		logoSpr = new FlxSprite(0, FlxG.height * 0.4).loadGraphic(Paths.image('menus/title/titlelogo'));
		add(logoSpr);
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				case 6:
					...
					addMoreText('Cehroaza', 45);
				case 8:
					deleteCoolText();
				case 9:
					createCoolText(['Rain World By'], -60);
				case 10:
					addMoreText('', -60);
