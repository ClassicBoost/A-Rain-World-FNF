...
class PlayState extends MusicBeatState
{
	...
	public static var disableScore:Bool = false;
	...
	public function createShit() {
		composerTxt = new FlxText(20, FlxG.height - 48, 0, SONG.song + (SONG.composer != null && SONG.composer != '' ? " - " + SONG.composer : ''), 12);
	...
	override public function update(elapsed:Float)
	{
		...
		if (disableScore) songScore = 0;

		botplay = (boyfriendStrums.autoplay || disableScore);
		...
		if (!inCutscene)
		{
			...
			if (health <= 0 && startedCountdown)
			{
				...
				FlxG.sound.play(Paths.sound('damage/fnf_loss_sfx' + GameOverSubstate.stageSuffix));
	...
	function goodNoteHit(coolNote:Note, character:Character, characterStrums:Strumline, ?canDisplayJudgement:Bool = true)
	{
		if (!coolNote.wasGoodHit)
		{
			...
			if (boyfriendStrums.autoplay) disableScore = true;
